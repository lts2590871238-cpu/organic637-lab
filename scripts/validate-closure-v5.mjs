...
// v5: true-exam bridges and official-outline coverage
for(let d=1;d<=20;d++){
 const day=D.days[d];assert(day.examBridge,`Day${d} missing examBridge`);
 for(const row of day.lessons||[])assert(row.examBridge,`Day${d} lesson ${row.id} missing examBridge`);
 const all=[...(day.questions||[]),...Object.values(day.repairs||{}).flat(),...(day.adaptivePool||[]),...Object.values(day.adaptivePools||{}).flat()];
 for(const row of all)assert(row.examBridge,`${row.id} missing examBridge`);
}
const corpus=Object.values(D.days).flatMap(day=>(day.lessons||[]).map(lessonText)).join(' ');
for(const required of ['自由基卤代','环张力','苯酚','Beckmann','杂环','葡萄糖','两性离子','肽键'])assert(corpus.includes(required),`official-outline coverage missing ${required}`);
for(const skill of ['alkane.radical_halogenation','cycloalkane.ring_strain','phenol.acidity','carbonyl.beckmann','heterocycle.aromaticity','carbohydrate.monosaccharide','aminoacid.zwitterion','aminoacid.peptide_bond'])assert((D.SKILLS||[]).some(x=>x.id===skill),`missing v5 skill ${skill}`);
assert(D.COURSE_SLOGAN==='学懂有机，会做真题','slogan missing');
assert(D.days[19].questions.reduce((s,q)=>s+Number(q.points||0),0)===150,'Day19 not 150 points');
console.log(`CLOSURE V5 PASS: ${lessonTotal} lessons; ${questionTotal} main/repair/adaptive question records checked; glossary ${G.length}; taught glossary ${firstLesson.size}; whyChain ${withWhy}/${lessonTotal}; storyboard ${withSequence}; micro-check ${withMicro}; analogy ${withAnalogy}; no planned-term prerequisite leakage.`);
