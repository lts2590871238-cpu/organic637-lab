...
const loads=['data/skills.js','data/manifest.js',...Array.from({length:20},(_,i)=>`data/day${String(i+1).padStart(2,'0')}.js`),'data/phase5-augment.js','data/beginner-foundations.js','data/learning-scaffolds.js','data/beginner-first-use.js','data/beginner-closure-v3.js','data/beginner-closure-v4.js','data/exam-closure-v5.js','data/teaching-closure-v7.js','data/detective-cases.js','data/synthesis-cases.js','js/chem-visuals.js'];
...
assert(D.COURSE_SLOGAN==='学懂有机，会做真题','slogan mismatch');assert(D.TEACHING_CLOSURE_VERSION==='v7-mainline','v7 marker missing');assert(D.days[19].questions.reduce((s,q)=>s+Number(q.points||0),0)===150,'Day19 !=150');
console.log(`CLOSURE V7 PASS: ${lessons} lessons; storyboard ${story}/${lessons}; micro-check ${micro}/${lessons}; hero diagrams ${hero}/${lessons}; ${questions} questions translated ${translated}/${questions}; glossary ${G.length}, first-use taught ${first.size}; prerequisite leakage audit PASS.`);
