...
require('../../data/v16-test-config.js');
require('../../data/v16-director.js');
require('../../js/director-engine.js');
require('../../js/comic-renderer.js');

const NS = window.Organic637;
assert.equal(NS.V16_TEST_CONFIG?.allDaysUnlocked, true, 'test build must unlock all days for QA');
...
assert.match(appSource, /V16_TEST_CONFIG[^\n]*allDaysUnlocked|allDaysUnlocked[^\n]*V16_TEST_CONFIG/, 'day unlocking must honor the test-build all-days switch');
...
assert.match(appSource, /TEST BUILD[^<\n]*20天全解锁/, 'test build must visibly announce that all 20 days are unlocked');
assert.match(appSource, /V16_TEST_CONFIG\?\.showTestBadge/, 'test build badge must be controlled by config, not hard-coded for formal release');
assert.match(appSource, /一份消失的样品|被改写的记录|LAB-20/, 'test-build welcome must establish the mystery instead of generic course copy');
...
console.log('PASS test-v16-test-build-immersion');
