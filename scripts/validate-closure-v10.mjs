...
assert(D.COURSE_SLOGAN==='学懂有机，会做真题','slogan mismatch');assert(D.TEACHING_CLOSURE_VERSION==='v10-mainline-deep-complete','v10 marker missing');assert(D.days[19].questions.reduce((s,q)=>s+Number(q.points||0),0)===150,'Day19 !=150');
console.log(`CLOSURE V10 BASE PASS: ${lessons} lessons; storyboard ${story}/${lessons}; micro-check ${micro}/${lessons}; hero diagrams ${hero}/${lessons}; ${questions} questions translated ${translated}/${questions}; glossary ${G.length}, first-use taught ${first.size}; prerequisite leakage audit PASS.`);
